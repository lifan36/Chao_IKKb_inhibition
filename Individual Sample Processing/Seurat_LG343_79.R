...
LG343_79.counts <- Read10X(data.dir = "/Users/lifan/Box Sync/Li_Fan_Research/10x/ChaoWang_IKKb/LG343_79/filtered_feature_bc_matrix")
LG343_79 <- CreateSeuratObject(counts = LG343_79.counts, project = "LG343_79", min.cells = 3, min.features = 200)
LG343_79[["Condition"]] = c('IKKbKO; Cre/+; PS19/+')
...
all
##An object of class Seurat 
##22387 features across 12207 samples within 1 assay 
##Active assay: RNA (22387 features)
#normalize counts=====
...
nExp_poi <- round(0.076*12207) #estimate the number of multiplets you expect from the kit you are using - should give you percent expected based on number of nuclei inputs
...
all <- doubletFinder_v3(all, PCs = 1:10, pN = 0.25, pK = 0.005, nExp = nExp_poi.adj, reuse.pANN = "pANN_0.25_0.005_928", sct = FALSE)
...
all <- RunTSNE(object = all, dims = 1:20, do.fast=TRUE, perplexity=100, max.iter=10000)
...
rm(all)
singlets<-saveRDS(singlets,"LG343_79_singlets.rds")
...
write.csv(singlets.markers,"LG343_79_cluster_markers_singlets.csv")

singlets<-saveRDS(singlets,"LG343_79_singlets_PCA.rds")
