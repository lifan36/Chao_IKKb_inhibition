...
LG343_116.counts <- Read10X(data.dir = "/Users/lifan/Box Sync/Li_Fan_Research/10x/ChaoWang_IKKb/LG343_116/filtered_feature_bc_matrix")
LG343_116 <- CreateSeuratObject(counts = LG343_116.counts, project = "LG343_116", min.cells = 3, min.features = 200)
LG343_116[["Condition"]] = c('IKKbKO; Cre/+')
...
LG343_116[["percent.mt"]] <- PercentageFeatureSet(object = LG343_116, pattern = "^mt-") #recognize mitochondrial transcripts
...
pdf("LG343_116_ggplot_pK.pdf", width=18, height=6)
ggplot(bcmvn_pbmc, aes(x=bcmvn_pbmc$pK, y=bcmvn_pbmc$BCmetric))+geom_bar(stat="identity") #look for pK at the initial peak
dev.off()
saveRDS(all,"LG343_116_PCA.rds") #it's good to save your R object periodically so you can start from this object without having to go through the processing steps again.
all<-readRDS("LG343_116_PCA.rds")
all
...
nExp_poi <- round(0.054*6775) #estimate the number of multiplets you expect from the kit you are using - should give you percent expected based on number of nuclei inputs
...
all <- doubletFinder_v3(all, PCs = 1:10, pN = 0.25, pK = 0.04, nExp = nExp_poi.adj, reuse.pANN = "pANN_0.25_0.04_366", sct = FALSE)
...
Idents(object = all) <- "DF.classifications_0.25_0.04_366" #visualizing the singlet vs doublet cells
DimPlot(object = all, reduction = 'umap')
...
rm(all)
singlets<-saveRDS(singlets,"LG343_116_singlets.rds")
singlets<-readRDS("LG343_116_singlets.rds")
...
write.csv(singlets.markers,"LG343_116_cluster_markers_singlets.csv")

singlets<-saveRDS(singlets,"LG343_116_singlets_PCA.rds")
